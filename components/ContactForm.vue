<template>
    ...
    <div class="isolate overflow-hidden bg-white px-4 py-16 sm:px-6 lg:px-8">
      <div class="relative mx-auto max-w-7xl">
        <div class="absolute inset-x-0 -top-20 -z-10 transform-gpu overflow-hidden blur-3xl sm:-top-60"
             aria-hidden="true">
          <div class="relative left-[calc(50%-11rem)] aspect-[1155/678] w-[36.125rem] -translate-x-1/4 rotate-[30deg] bg-gradient-to-tr from-green-200 to-teal-200 opacity-30 sm:left-[calc(50%-30rem)] sm:w-[72.1875rem] polygon" />
        </div>  
      </div>
      <div class="relative mx-auto max-w-xl">
        <div>
          ...
          <p class="mt-4 text-base leading-6 text-gray-500">
            <slot name="subtitle">
              ...
              <a class="font-semibold text-green-700 no-underline"
                 href="/faq/"> FAQ </a> page to see if your question has already been answered. We look forward to hearing from you!
            </slot>
          </p>
        </div>
        <p class="my-2">
          <span class="text-red-700">*</span> indicates a required field
        </p>
        <div class="mt-8">
          <form action="/.netlify/functions/contact-form"
                method="POST"
                class="grid grid-cols-1 gap-y-6 sm:grid-cols-2 sm:gap-x-8"
                @submit.prevent="submit">
            <div class="sm:col-span-2">
              <label for="name"
                     class="block text-sm font-semibold text-gray-700">Name <span class="text-red-700">*</span></label>
              <div class="mt-1">
                <input id="name"
                       v-model="form.name"
                       type="text"
            ...
            <div class="sm:col-span-2">
              <label for="email"
                     class="block text-sm font-semibold text-gray-700">Email <span class="text-red-700">*</span></label>
            ...
            <div class="sm:col-span-2">
              <label for="category"
                     class="mb-1 block text-sm font-semibold text-gray-700">Category <span class="text-red-700">*</span></label>
              <select id="category"
                      v-model="form.category"
                      class="block w-full rounded-md border-gray-300 px-3 py-3 shadow-sm focus:border-green-500 focus:ring-green-500">
            ...
            <div v-if="form.category == 'Volunteer'"
                 class="sm:col-span-2">
              <div class="rounded-md bg-green-50/60 p-4">
                <div class="flex">
                  <div class="flex-shrink-0">
                    <Icon name="heroicons:exclamation-circle"
                          class="mt-1 h-6 w-6 text-green-800"
                          aria-hidden="true" />
                  </div>
                  <div class="ml-3">
                    <h3 class="text-xl font-medium text-green-800">
            ...
            <div class="sm:col-span-2">
              <label for="message"
                     class="block text-sm font-semibold text-gray-700">Message <span class="text-red-700">*</span></label>
              <div class="relative mt-1 rounded-md shadow-sm">
                <textarea id="message"
            ...
            <div class="sm:col-span-2">
              <span class="inline-flex w-full rounded-md shadow-sm">
                <button :disabled="status === 'pending'"
                  ...
                  <span>
                    <Icon v-if="status === 'pending'"
                          name="mdi:loading"
                          class="-ml-1 mr-2 h-6 w-6 animate-spin text-white" />
                  </span>
                  Submit
                </button>
              </span>
        ...
        <!-- notification banner -->
        <div v-if="status === 'success' || error !== null"
             class="my-8 rounded-md p-4"
          ...
          <div class="flex">
            ...
            <div class="ml-3">
              <h3 :class="[error ? 'text-red-800' : 'text-green-800', 'text-lg font-medium']">
                {{ error ? 'There were errors with your submission' : 'Thank you for contacting us!' }}
              </h3>
  ...
  const { status, data, error, execute } = useFetch('/.netlify/functions/contact-form', { immediate: false, watch: false, method: 'POST', body: form.value });
  
  function submit() {
    if (form.value.name === '' || form.value.email === '' || form.value.message === '') {
      displayMessage.value = 'Please fill out the form completely';
      return;
    }
    execute();
  }
