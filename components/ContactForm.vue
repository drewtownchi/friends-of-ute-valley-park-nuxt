    ...
    <div class="isolate overflow-hidden bg-white px-4 py-16 sm:px-6 lg:px-8">
      ...
      <div class="relative mx-auto max-w-xl">
            ...
            <div class="sm:col-span-2">
              <span class="inline-flex w-full rounded-md shadow-sm">
                <button
                  ...
                  <span>
                    <Icon v-if="isFetching" name="mdi:loading" class="-ml-1 mr-2 h-6 w-6 animate-spin text-white" />
                  </span>
                  Submit
                </button>
              </span>
        ...
        <div v-if="isFinished" class="my-8 rounded-md p-4" :class="[error ? 'bg-red-50' : 'bg-green-50']">
          <div class="flex">
            <div class="flex-shrink-0">
              <Icon name="heroicons:megaphone" class="size-6" :class="[error ? 'text-red-800' : 'text-green-800']" />
            </div>
            <div class="ml-3">
...
const { isFetching, isFinished, data, error, execute } = useFetch("/.netlify/functions/contact-form", {
  immediate: false,
})
  .post({
    payload: form.value,
